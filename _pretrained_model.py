...
class SwinTransformerPretrainedClassifier(BaseTorchPretrainedImageClassifier):
  ...
  def _prepare_model(self):
    ...
    architecture_map = {"swin_t": swin_t, "swin_s": swin_s, "swin_b": swin_b}

    model_fn = architecture_map.get(self.architecture)
    self._model = model_fn(weights="IMAGENET1K_V1")
    num_features = self._model.head.in_features
    if self.n_classes_ == 2:
      self._model.head = torch.nn.Linear(num_features, 1)
    else:
      self._model.head = torch.nn.Linear(num_features, self.n_classes_)
...
class VisionTransformerPretrainedClassifier(BaseTorchPretrainedImageClassifier):
  ...
  def _prepare_model(self):
    ...
    if self.n_classes_ == 2:
      self._model.heads.head = torch.nn.Linear(num_features, 1)
    else:
      self._model.heads.head = torch.nn.Linear(num_features, self.n_classes_)
